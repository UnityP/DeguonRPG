...
{
    public class RoguelikeGeneratorMap : MonoBehaviour
    {
        #region Variables
        ...
        // 패턴 바닥 오버레이 타일을 그려야 하는지 여부를 나타내는 플래그입니다. true인 경우 패턴 오버레이 타일이 바닥에 그려집니다.
        public bool drawFloorOverlayPatternTiles = true; 

        // 이것은 패턴 바닥 타일의 GameObject입니다. 이 타일 객체는 패턴 바닥 오버레이 타일을 그릴 때 사용됩니다.
        public GameObject patternFloorTileObj; 

        // 임의의 바닥 오버레이 타일을 그려야 하는지 여부를 나타내는 플래그입니다. 참이면 무작위 오버레이 타일이 바닥에 그려집니다.
        public bool drawFloorOverlayRandomTiles = true; 

        // 이것은 임의의 바닥 타일에 대한 GameObject입니다. 이 타일 개체는 임의의 바닥 오버레이 타일을 그릴 때 사용됩니다.
        ...
        /// 애플리케이션이 실행 중인 경우, GameObject.Destroy를 사용하여 안전하게 자식 게임 오브젝트를 제거합니다.
        /// 애플리케이션이 실행 중이 아닌 경우(에디터에서 실행 중), GameObject.DestroyImmediate를 사용하여 즉시 자식 게임 오브젝트를 제거합니다. 
        /// </summary>
        private void Clear()
        {
